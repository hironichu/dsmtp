...
try {
  ...
  const res = prompt(
    "Say OK to regenerate the key, warning this will delete all the data\n\r",
  );
  if (res === "OK") {
...
const PASSWORD = prompt(
  "Enter a secret phrase to generate the key (please make it dificult to recreate!) \n\r",
);
if (!PASSWORD) {
...
localStorage.setItem(
  "salt",
  btoa(salt.reduce((data, byte) => data + String.fromCharCode(byte), "")),
);
localStorage.setItem(
  "iv",
  btoa(iv.reduce((data, byte) => data + String.fromCharCode(byte), "")),
);
...
const data = await window.crypto.subtle.encrypt(
  { name: "AES-GCM", iv: iv },
  DERIVED_KEY,
  new TextEncoder().encode(smtpData),
);
...
try {
  ...
  const data_decoded = await window.crypto.subtle.decrypt(
    { name: "AES-GCM", iv: new Uint8Array(iv_s) },
    keydata_import,
  ...
  console.error(
    "Error while decrypting the data, deleting the key and the data",
  );
  Deno.removeSync("./server.key");
  localStorage.clear();
  console.error(e);
